...
In this appendix, we derive the result used to speed up our implementation of \Cref{outerloop} in Section \Cref{maxwell results}.
...
\end{align}
Since the primal residual, as we as both the primal and enriched eigenspaces, depends only on $x,$ the DWR error estimate \eqref{DWR} depends only on $x.$ Thus,
% depends only on $x,$ as well as both the primal and enriched eigenspaces, the DWR error estimate \eqref{DWR} depends only on $x.$ Thus,
\begin{align}
