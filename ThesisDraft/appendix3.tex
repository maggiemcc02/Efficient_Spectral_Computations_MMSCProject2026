\chapter{Speeding Up Compuations for the Maxwell Example}\label{speedup}


In this appendix, we derive the result used to speed up our implementation of Algorithm \ref{outerloop} in Section \ref{maxwell results}.

Let $A$ be a self-adjoint, closed, and densely defined operator on the Hilbert space $H.$  For Colbrook's algorithm, we consider the folded operator $F(z) \defeq (A-zI)^*(A-zI).$ Since $A^*=A,$ we have
...
\begin{equation}
    Q_{z, A}(u, v) \defeq ((A-zI)u, (a-zI)v)_H,
\end{equation}
for all $u, v \in D(A).$
Substituting in $z = x+iy,$ we have
\begin{align}
    Q_{z,A}(u,v)
    &= \bigl((A-xI)u-iyu,\,(A-xI)v-iyv\bigr)_H \\
    &= \bigl((A-xI)u,(A-xI)v\bigr)_H
       + iy\bigl((A-xI)u,v\bigr)_H \notag\\
    &\quad
       - iy\bigl(u,(A-xI)v\bigr)_H
       + y^2(u,v)_H \\
    &= \bigl((A-xI)u,(A-xI)v\bigr)_H
       + y^2(u,v)_H \\
    &= Q_{x,A}(u,v)+y^2(u,v)_H.
\end{align}
Taking $v=u,$ we have that
\begin{align}
    Q_{z,A}(u,u)
    &= Q_{x,A}(u,u)+y^2\|u\|_H^2.
\end{align}
Then, taking infimum over unit vectors, we can write \eqref{gamma} as
\begin{equation}
    \gamma(z, A)^2 = \gamma(x, A)^2 + y^2. 
\end{equation}
...
\begin{align} \label{xval}
    Q_{x, A}(u_h(x), v_h) &= (\lambda_h(x) - y^2) m(u_h(z), v_h) = \lambda_h(x) m(u_h(x), v_h), 
\end{align}
for all $v_h \in V_h.$
...
With this, we can conclude that for $z$ values that share the same real part, Colbrook's bound on the distance to the spectrum depends only on a shift by the square of the imaginary part:
