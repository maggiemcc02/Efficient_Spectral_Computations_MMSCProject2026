...
\end{align}
Making the $z$-dependence explicit, the auxiliary eigenproblem, \eqref{max aux}, seeks $\lambda_h(z)$ and nonzero $u_h(z) \in V_h$ that satisfy
\begin{equation} \label{zval}
    Q_{z, A}(u_h(z), v_h) = \lambda_h(z) m(u_h(z), v_h).
\end{equation}
for all $v_h \in V_h.$
By \eqref{C10}, this is equivalent to seeking $\lambda_h(x)$ and nonzero $u_h(x) \in V_h$ that satisfy
\begin{align} \label{xval}
    Q_{x, A}(u_h(x), v_h) &= (\lambda_h(z) - y^2) m(u_h(x), v_h) = \lambda_h(x) m(u_h(x), v_h), 
\end{align}
for all $v_h \in V_h.$
...
\end{align}
where $\lambda(z)$ is the eigenvalue that $\lambda_h(z)$ approximates.
