...
\begin{equation}
	a(u,v) \coloneqq \langle (L-zM)^* M^{-1} (L-zM) u, v \rangle_{(V^*,V)} = (Au, Av)_{L^2(\Omega)}.
\end{equation}

Let us now take a closer look at the operator $G:V\to V^*$. This operator is perfectly well-defined (once again thank to the double dual identification) if $(L-zM)$ goes from $V$ to $H^*$:
\begin{equation}
	V \overset{(L-zM)}{\longrightarrow} H^* \subset V^* \overset{M^{-1}}{\longrightarrow} V \subset H \sim (H^*)^* \overset{(L-zM)^*}{\longrightarrow} V^*.
\end{equation}
This setting works for the following scenarios:
\begin{enumerate}
	\item Dirac folding, $V=H^1 (\Omega) \times H^1_0(\Omega)$ and $H=L^2(\Omega)$;
	\item First order Maxwell folding, $V=H_0(\text{curl};\,\Omega)\cap H(\text{div}; \Omega) \times H_0(\text{curl};\,\Omega)\cap H(\text{div}; \Omega)$ and $H=L^2(\Omega)$;
	\item Laplace folding, $V=H^1_0(\Omega)\cap H^2(\Omega) $ and $H=L^2(\Omega)$, with $\Omega$ convex.
\end{enumerate}
Notice that in all this cases we have $V\subset\!\subset H$, hence we can introduce (with no use of elliptic regularity !) the solution operator associated to $G:V\to V^*$, i.e.
\begin{equation}
	T: H \to H, \qquad a(Tf,v) = (f,v)_H, \qquad \forall v \in V,
\end{equation}
which in turns allow us to make use of the Babuska-Osborn theory perfectly explained in Boffi's acta on the pair $(G,T)$.
\end{document}
