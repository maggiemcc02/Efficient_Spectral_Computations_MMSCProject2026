...
This approach comes with various issues, first of it is not familiar to people in FEM land, but more importantly  the operator is unbounded and its solution operator does not actually fit the Babuska-Obsorn theory that we would like to use to have a priori error estimates. This because this theory relay on the painr $(G, T)$ of a bounded linear operator $G: V \to V^*$ and its corresponding \underline{compact} solution operator $T:H \to B \subset H$.

For this reason we would like propose a different functional analytical view point, that rests on the following identity:
\begin{equation}
	M^{-1}(L-zM)=(M^{-1} - z) \Rightarrow G\coloneqq(L-ZM)^* M^{-1} (L-zM) = (M^{-1}L-z)^* M^* (M^{-1}L - z),
\end{equation}
notice that everything make sense because we have made use of the canonical double dual identification (which is not a \underline{crime} contrary to the Riesz dual identification, see Brezzi--Boffi--Fortin). The previous equation tell us something very interesting, both folding the $A$ endomorphism and the operator $G:V\to V^*$, are associated to the same bilinear form (that we can assemble in Firedrake !), i.e.
\begin{equation}
