...
Above we have considered the operator $M^{-1}L$ where $L$ is the ``weak'' Laplacian and $M$ is the $L^2$ Riesz map, i.e.
\begin{equation}
...
\end{align}
Here the primes denote dual operators; they are not the Hilbert-space adjoint
of the unbounded operator $A$. The previous equation tells us something very
interesting: both the folding of the endomorphic $A$ and the operator
$G:V\to V^*$ are associated to the same bilinear form (that we can assemble
in Firedrake!), i.e.
\begin{equation}
	a_z(u,v) \coloneqq \langle C_z'MC_z u,v\rangle_{(V^*,V)}
	= \bigl(C_z u,C_z v\bigr)_{\mathcal{H}}
	= \bigl((A-zI)u,(A-zI)v\bigr)_{\mathcal{H}}.
\end{equation}

Let us now take a closer look at the operator $G:V\to V^*$. This operator is perfectly well-defined (once again thank to the double dual identification) if $(L-zM)$ maps from $V$ to $\mathcal{H}^*$:
\begin{equation}
