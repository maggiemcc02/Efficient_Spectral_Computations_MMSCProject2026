...
we have
\begin{align}
	G \coloneqq (L-zM)^*M^{-1}(L-zM)
	&= \bigl(M(A-zI)\bigr)^*M^{-1}M(A-zI) \notag\\
	&= (A-zI)^*M^*M^{-1}M(A-zI) \notag\\
	&= (A-zI)^*M^*(A-zI)
	= (M^{-1}L-zI)^*M^*(M^{-1}L-zI). \notag
\end{align}
Notice that everything make sense because we have made use of the canonical double dual identification (which is not a \underline{crime} contrary to the Riesz dual identification, see Brezzi--Boffi--Fortin). The previous equation tells us something very interesting: both the folding of the endomorphic $A$ and the operator $G:V\to V^*$ are associated to the same bilinear form (that we can assemble in Firedrake!), i.e.
\begin{equation}
	a_z(u,v) \coloneqq \langle (L-zM)^* M^{-1} (L-zM) u, v \rangle_{(V^*,V)} = \bigl((A-zI)u, (A-zI)v\bigr)_{L^2(\Omega)}.
\end{equation}
...
\begin{equation}
	V \overset{(L-zM)}{\longrightarrow} \mathcal{H}^* \subset V^* \overset{M^{-1}}{\longrightarrow} V \subset \mathcal{H} \sim (\mathcal{H}^*)^* \overset{(L-zM)^*}{\longrightarrow} V^*.
\end{equation}
