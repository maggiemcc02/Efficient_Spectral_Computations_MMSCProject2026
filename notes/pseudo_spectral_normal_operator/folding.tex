...
\usepackage{tikz-cd}
% hypertexnames=false: the \tag'd equations (*) and (**) do not step the equation
% counter, so counter-based anchors collide.  Sequential names avoid the clash.
\usepackage[colorlinks=true,linkcolor=blue!55!black,citecolor=blue!55!black,
            urlcolor=blue!55!black,hypertexnames=false]{hyperref}
\usepackage[nameinlink]{cleveref}
\usepackage{todonotes}
\newcommand{\pef}[1]{\todo[inline, color=green!40]{PEF: #1}}
\newcommand{\MM}[1]{\todo[inline, color=pink!70]{MM: #1}}
\newcommand{\uz}[1]{\todo[inline, color=teal!30]{UZ: #1}}
\newcommand{\uzt}[1]{\textcolor{teal!80!black}{#1}}   % corrected text, in place
...
\title{\vspace{-1.5em}The functional analytic aspects of operator folding\\
       and variational formulations\vspace{-0.5em}}
...
the context of variational formulations (and their discretisations, like the finite element method).

\pef{I propose we rephrase to that there's a problem with variational formulations, not with the finite element method. It arises on the continuous level also.}
\uz{I would be happy with this rephrasing.}
\pef{I confess I find this note quite difficult to read. Umberto, how much of it was written with Claude? I find its prose with Opus 5 to be much, much worse than previous versions.}
\uz{This is minor edits from what Claude wrote after parsing my hand written notes, I just checked that the contact of my hand written notes was there and didn't worry about how it embellished it. For the maths I would consider the notes I wrote and shared over slack and the one me and Maggie have been writing these weeks as the source of truth.}
\smallskip
\noindent\textbf{(a)}\ \emph{It is not straightforward to apply the algorithm as stated in
\cite[\S3.4.1]{Colbrook2026} in the context of a variational formulation.}  The algorithm
...
\smallskip
\noindent\textbf{(b)}\ \emph{What is \uzt{computed} for Maxwell in
\cite[\S3.4.4]{Colbrook2026} is not that algorithm.}  \uzt{The minimisation performed
there is, we believe, a conforming Rayleigh--Ritz discretisation of the pencil
$\eqref{eq:starstar}\,u = \lambda M u$ on $V \times V$, the formulation derived in
\cref{sec:path}.}  \Cref{sec:already} gives our evidence.
\uzt{If (b) is right, the algorithm \emph{as stated} in \cite[\S3.4.1]{Colbrook2026}
and the computation \emph{as performed} in \cite[\S3.4.4]{Colbrook2026} are two
descriptions of one object, and what is missing from \cite{Colbrook2026} is the
passage between them, which is \cref{sec:path}.}
...
\pef{Have you checked the final version of the book? And mention what $\Phi_n$ is.}
\MM{PEF, do you have a final copy?}
\uz{I have checked the verision of the book Matt shared with me.}

We cannot find in the book what these
...
\end{equation}
the first half being needed for $\uzt{\Phi}$ to be defined at all.  On that reading
one and the same finite-dimensional space must lie in \emph{both} domains (of $A$ and $A^*$), and
those carry different boundary conditions in general.  \uzt{If instead those
conditions are meant to make \eqref{eq:folded} well posed, then they are conditions
of a different kind.}
\pef{What does it mean for \eqref{eq:folded} to be well-posed? It's just an operator, not an equation or a problem.}
\uz{If an operator goes from V to W and you compose with an operator going from X to Y, it is ill-posed.}
...
\paragraph{\uzt{(ii) No error estimate in terms of the best approximation of the
space.}}
\uzt{Convergence in Matt's book comes from a core assumption, that the trial spaces
exhaust $\dom(A)$ in the graph norm, so $\Phi \downarrow \gamma$, while conformity is
what gives $\Phi \ge \gamma$.  Neither tells me how well the space I actually use
approximates anything, and every finite element estimate I know is of the form
$\text{error} \lesssim \inf_{v_h \in V_h} \norm{u - v_h}$.  To get one I need the
bottom of the folded spectrum to be attained, so that there is a minimiser to
approximate, and then Babu\v{s}ka--Osborn \cite{BabuskaOsborn1991,Boffi2010} to turn
best approximation of the eigenspace into a bound on the eigenvalue.  Matt assumes
neither: \cite[p.~129]{Colbrook2026} does not assume a compact resolvent, and
\cite[Prop.~3.3.12]{Colbrook2026} gives a rate only for a \emph{normal} $A$ with an
isolated eigenvalue and a computable gap, which is not the case for the problem we are
interested in.  This is why I believe we need Babu\v{s}ka--Osborn theory.  I am not
saying any of this is wrong, it is deliberate; I am saying that a certificate is not
an error estimate, and without one I have nothing to refine on.}
...
\MM{I agree. My (potentially incorrect) interpretation is as follows: Matt considers the Hilbert adjoint. However, when we consider an operator $A: V \rightarrow V^*,$ the natural notion of the adjoint is the Banach adjoint. Then, we map from the Banach adjoint to the Hilbert adjoint (and thus back to Matt's setting) with the appropriate Riesz map. I believe this is what Umberto does in Section 3?}
\uz{Yes, that would be the reply, but I believe it misses the point.  A finite element method never produces an operator
$\Hb \to \Hb$ for anyone to take the Hilbert adjoint of.  What assembly gives me is a
bilinear form, that is $A \colon V \to V^*$, and the Hilbert adjoint doesn't help.
This is the point we will also raise in (iv), that
finite element operators go into $\Hb^*$ and not $\Hb$. Furthermore notice that the
Hilbert adjoint is the Banach adjoint with the Riesz map, $A^* = M^{-1}A'M$, so
invoking it does not remove $M$, it fixes \emph{which} $M$, namely that of the pivot
space $\Hb$ in which $\siginf$ is measured and not that of $V$.  For the \emph{first-order} Maxwell
system $A$ maps into $\Hb^* \subseteq V^*$, so $M^{-1} \colon \Hb^* \to \Hb$ is
available and \eqref{eq:starstar} has a variational form, while for $\Delta$ on
CG$_1$ the image is not in $\Hb^*$ and there is no repair, which is (v) below.}
...
\paragraph{(v) The trial space must be a core in the graph norm.}
The structural difficulty is that folding evaluates
...
\uzt{three} things then go wrong at once.  Continuous piecewise linears are not in
$H^2(\Omega)$, so $\norm{(A - zI)u_h}_{L^2}$ is undefined, and
...
graph norm, so no $H^2$-conforming family is a core. Furthermore $\dom(A^*)$ carries different boundary
...
functions, which are smooth, with no boundary (this is the whole reason why the advection-diffusion operator is not normal \dots). 
Nor in \cite[\S3.4.4]{Colbrook2026}, whose operator is the \emph{first-order} Maxwell
...
\paragraph{(vi) The estimator we want needs more than either.} 
\pef{Keep the note strictly focused on Colbrook's work, what we want to do
after is not important here. I suggest getting rid of this paragraph altogether.}
\uz{I believe you are right !}
We intend to
...
certificate survives, but the estimator has no anchor. Whether the
bottom is isolated also depends on where $z$ sits, so the inner and outer layers
of adaptivity are not independent.
\MM{This is unfortunately the strong assumption that we make to apply DWR. I view this as a limitation in our choice of DWR rather than an issue  with Matt's work.  Hopefully we can work on this in the future. It would be really interesting to develop an error estimate for a general spectral bottom.}
% ═════════════════════════════════════════════════════════════════════════════
\section{The setting we propose}
\label{sec:path}

We can so far only make sense for first-order operators, so we restrict ourselves to this setting.

Let $V \subset\subset \Hb$ with $\Hb = L^2$. For concreteness consider the Maxwell eigenproblem, and thus take $V$ of the type $H_0(\curl)
...
\cite[footnote~2, p.~130]{Colbrook2026}.} \pef{This doesn't make any sense as a general setting to propose.}
\uz{It is not a general setting it is a concrete example. Why do you think this doesn't make any sense ?}
From here $A$ is the \emph{weak}
operator \pef{What does that mean? We're trying to explain things here.}\uz{Here I mean an opeartor defined to a weak formulations, i.e. a variational formulation, so it spit what we call a load-vector not a vector or a function this is an element of the dual.} related to the endomorphism of \cref{sec:setting} by $\Astr = M^{-1}A$,
...
double dual identification, numerically an identity. \pef{Spell this out.} This is the continuous
...
\MM{The assumption that we are working with the first-order operator is obviously not great. However, this seemed like the most straightforward way to directly relate our variational folding of the Maxwell (and the Poisson) problems to what Matt writes in his book. Umberto and I tried to work this out in a more general setting and kept getting stuck.}
\uz{Understanding this things is hard, I would not be too hard on the fact that we only understood the first order formulations.}
...
while a finite element method produces $A \colon V \to V^*$, \uzt{this is not an endomorphism for this reason it is typical in finite element land to use the solution operator trick.}

A G\aa rding inequality can be proven and will be useful with the hypothesis $V$ compactly included in $\Hb$ to give that the solution operator we are considering if Fredholm of index 0. Applying $(s-t)^2 \ge \tfrac12 s^2 - t^2$ with $s = \norm{\Astr
...
eigenvalues are reciprocal to those of the pencil, $\lambda = \mu^{-1} - C$, so we have a
discrete spectrum $0 \le \lambda_1 \le \lambda_2 \le \cdots
