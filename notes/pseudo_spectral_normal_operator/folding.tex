...
\noindent
In \cite{Colbrook2026} Colbrook introduces a very appealing algorithm for
computing spectra, and more interestingly pseudospectra, of differential
operators.  This note makes two claims about its relation to the finite element
method.

\smallskip
\noindent\textbf{(a)}\ \emph{The algorithm as stated in
\cite[\S3.4.1]{Colbrook2026} cannot be followed by a finite element method.}  It
is phrased in terms of $A$, $A^*$, $\dom(A)$, $\dom(A^*)$ and orthogonal
projections $\mathcal{P}_n$, and a finite element code has none of those.  It has
a bounded sesquilinear form on $V \times V$.  \Cref{sec:stuck} lists six places
where the passage from one to the other breaks down.
...
\noindent If (b) is right the two are reconciled, and what is missing from
\cite{Colbrook2026} is the bridge between them, which is \cref{sec:path}.

% ═════════════════════════════════════════════════════════════════════════════
\section{The setting, as we read it}
\label{sec:setting}
...
H^2(\Omega) \cap H^1_0(\Omega) \to L^2(\Omega)$.  Under suitable conditions on
...
Assumption~3.2.1''.  That hypothesis carries everything, since convergence from
above is what Assumption~3.2.1 asks for and what turns
\cite[Thm~3.2.3]{Colbrook2026} into $\Gamma^\epsilon_n(A) \subseteq
\Sp_\epsilon(A)$, the error control.  We cannot tell what it is.  Reading back
...
one and the same finite-dimensional space must lie in \emph{both} domains, and
those carry different boundary conditions in general.  If instead the conditions
are meant to make \eqref{eq:folded} well posed, they are different ones.
...
\paragraph{(ii) Babu\v{s}ka--Osborn does not apply.}  We would like to apply
...
isolated eigenvalue and a computable gap.  Non-normality is the case
pseudospectra exist for.

\paragraph{(iii) A primal-dual mismatch.}  Read \eqref{eq:folded} as what it
looks like, a product of $A-zI$ with its adjoint.  The Banach adjoint is
$(A-zI)^* \colon \Hb^* \to \Hb^*$, so we would be composing $\Hb
\xrightarrow{A-zI} \Hb$ with $\Hb^* \xrightarrow{(A-zI)^*} \Hb^*$, and the two
halves do not meet.  The standard repair, when $\Hb = L^2$, is to identify $\Hb$
with its dual silently.  That does keep the composition an endomorphism.  We are
not fond of it.
