...
\MM{Is the point here that Umberto thinks that what Matt is doing in his Maxwell example can be explained in Section 3, which inserts the Riesz maps needed for folding in the variational setting?}
...
\MM{Is this saying that if our view of Matt's Maxwell example is correct, Section 3 shows that our implementation aligns with how Matt presents his algorithm?}
...
trace is not zero?}
\MM{I agree that this is not an endomorphism but that it is an example of $A:D(A) \subset \Hb \rightarrow \Hb.$}
...
Assumption~3.2.1''. \pef{Have you checked the final version of the book? And mention what $\Phi_n$ is.} \MM{PEF, do you have a final copy?}That hypothesis carries everything, since convergence from
...
\MM{Is this section highlighting the assumption we have to make to apply DWR? Namely, that the bottom of the spectrum of the folded operators must consist of an isolated eigenvalue of finite multiplicity? It is not clear to me why we want a convergence rate and error indicator from Babu\v{s}ka--Osborn theory?}
