...
\begin{theorem}[{Neumann Series \cite[7.3-1]{krey}}]  \label{neum}
    ...
    \begin{equation}
        (I - T)^{-1} = \sum_{j=0}^{\infty} T^j = I + T + T^2 + \dots,
    \end{equation}
...
\end{equation}
From this, we can see that when $|z| \ge \|T\| + 1,$ $\|R_T(z)\| \le 1.$ Furthermore, as $|z| \rightarrow \infty,$ $\|R_T(z)\| \rightarrow 0.$ Inside the disk, $|z| \le \|T\| + 1,$ $R_T(z)$ is analytic, and is thus continuous. Since this disk is compact, $R_T(z)$ is bounded on the disk\footnote{I am not sure if I actually need the $+1$ here or if the Neumann series is enough (Neumann series says $R_T(z)$ is bounded on the disk $|z| > \|T\|$)}. 
...
To conclude our introduction to pseudospectra, we state the following result for the spectra and pseudospectra of adjoints:
\begin{theorem}[{Spectra of the Adjoint \cite[Theorem 4.4, page 581, respectively]{Nick, Matt}}]
    Let $A$ be a closed and densely defined operator on the Banach space $X.$  Then, for any \(z \in \mathbb C\), and \(\varepsilon > 0\), we have
\[
\left\|(\overline{z}-A^*)^{-1}\right\|
=
\left\|(z-A)^{-1}\right\|,
\qquad
\sigma(A^*)=\overline{\sigma(A)},
\qquad
\sigma_{\varepsilon}(A^*)=\overline{\sigma_{\varepsilon}(A)}.
\]
Moreover, suppose \(A\) has an \(\varepsilon\)-pseudoeigenvector
\(u\in\mathcal D(A)\) corresponding to the \(\varepsilon\)-pseudoeigenvalue \(z\).
If \(z\notin\sigma(A)\), then \(A^*\) has an \(\varepsilon\)-pseudoeigenvector
\(f\in\mathcal D(A^*)\) corresponding to the \(\varepsilon\)-pseudoeigenvalue
\(\overline{z}\).
\end{theorem}
