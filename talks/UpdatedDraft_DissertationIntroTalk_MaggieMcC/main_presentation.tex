...
%     % chosen z: center of small ball
%     \coordinate (z) at (0,0);
%     \node[circle, fill=green!60!black, draw=green!40!black, inner sep=2.5pt] at (z) {};
%     \node[text=green!50!black] at (0.18,-0.26) {$z$};

%     % small ball B(z,E_n(z))
%     \draw[annotblue, thick] (z) circle (0.72);
%     \node[text=annotblue, font=\scriptsize] at (0.2,0.88) {$B(z,d_{n,z})$};

%     % local minimizer w_*
%     \coordinate (w) at (0,-0.6);
%     \node[circle, fill=orangeann, draw=orangeann, inner sep=2.5pt] at (w) {};
%     \node[text=orangeann] at (-0.3,-0.4) {$w_*$};

%     % point on the ball for the blue arrow
%     \coordinate (ballpt) at ($(z)+(25:0.72)$);

%     % % blue explanatory text
%     % \node[
%     %     text=annotblue,
%     %     align=left,
%     %     anchor=west,
%     %     font=\small
%     % ] (blueText) at (2.55,1.45)
%     % {locally minimize $\gamma(\cdot, A)$ over \\ $B(z,d_{n,z})\cap\text{Grid(M)}$.};

%     % % blue arrow: points to the ball
%     % \draw[->, annotblue, thick]
%     %     (blueText.west)
%     %     .. controls (2.15,1.55) and (1.35,1.25) ..
%     %     (ballpt);
...
%         anchor=west,
%         font=\small
%     ] (orangeText) at (2.55,-0.45)
%     {$w_* = \min_{w \in B(z,d_{n,z})\cap\text{Grid(M)}} \gamma(w, A)$ \\
%       is chosen for
%      the approximate spectrum.};

% % orange arrow: terminates on the lower-left part of the blue circle
% % endpoint on the blue circle, just to the right of the bottom
% % endpoint on the blue circle
% % endpoint on the blue circle, just to the right of w_*
% \coordinate (orangeend) at ($(z)+(280:0.8)$);

% % smooth orange arrow with head pointing up-left toward w_*
% \draw[->, orangeann, thick]
%     (orangeText.west)
%     .. controls (1.90,-0.5) and (0.9,-2) ..
%     (orangeend);
...
\begin{enumerate}
    \item To compute $d_{n,z},$ we solve eigenproblems at each $z \in \text{Grid}(M).$ This is expensive.
    \vspace{0.5em}
