...
% The standard approach in infinite dimensions is to discretize the problem, compute the spectra of the corresponding finite-dimensional operator, and hope that the set of computed eigenvalues approaches $\text{Sp}(A).$ 
...
The standard approach in infinite dimensions is to discretize $A$ with an $n$-dimensional operator, $A_h,$ and hope that $\text{Sp}(A_h)$ approaches $\text{Sp}(A).$
...
% \begin{frame}{The Algorithm: 1. Discretization}

% \begin{itemize}
%     \item To approximate $\operatorname{Sp}(A)$, first discretize the complex plane with a uniform grid covering a disc of radius $M$, denoted $\operatorname{Grid}(M)$.
% \end{itemize}
% % To approximate $\operatorname{Sp}(A)$, first discretize the complex plane with a uniform grid covering a disc of radius $M$, denoted $\operatorname{Grid}(M)$.

% \vspace{1em}

% \centering
% \tikzset{ axis/.style={thick, gray!70, -{Latex[length=2mm]}}, specpt/.style={circle, fill=red!80!black, inner sep=1.7pt}, gridpt/.style={circle, fill=black!55, inner sep=1.2pt}, testpt/.style={circle, fill=green!60!black, draw=green!50!black, inner sep=2pt}, minpt/.style={circle, fill=magenta!90, draw=magenta!90, inner sep=2pt}, disc/.style={dashed, gray!70, thick}, lab/.style={font=\small}, }
...
%     % labels
%     \node[black!55] at (1.05,1.82) {$\operatorname{Grid}(M)$};
%     \node[text=red!80!black, font=\small] at (-1.15,1.55) {$\operatorname{Sp}(A)$};
% \end{tikzpicture}

% \end{frame}

% \begin{frame}{The Algorithm: 2. Auxillary Eigenproblems}

% \begin{itemize}
%     \item For each $z \in \text{Grid}(M),$ compute an approximation $\Phi_n(z, A) \approx \gamma(z, A)$ and the associated bound on the distance to the spectrum, $d_{n,z}.$ 
%     \vspace{0.5em}
%     \item This often requires approximating the smallest eigenvalues of the $n$-dimensional discretizations of $(A-zI)^*(A-zI)$ and $(A-zI)(A-zI)^*.$
% \end{itemize}

% % For each $z \in \text{Grid}(M),$ compute an approximation of $\gamma(z, A),$ and the associated bound on $\text{dist}(z, \text{Sp}(A)),$ $d_{n,z}.$ 
...
% \tikzset{ axis/.style={thick, gray!70, -{Latex[length=2mm]}}, specpt/.style={circle, fill=red!80!black, inner sep=1.7pt}, gridpt/.style={circle, fill=black!55, inner sep=1.2pt}, testpt/.style={circle, fill=green!60!black, draw=green!50!black, inner sep=2pt}, minpt/.style={circle, fill=magenta!90, draw=magenta!90, inner sep=2pt}, disc/.style={dashed, gray!70, thick}, lab/.style={font=\small}, }
% \begin{tikzpicture}[scale=1]
% \path[use as bounding box] (-2.2,-2.1) rectangle (2.3,2.25);
%     % disc
%     \draw[disc] (0,0) circle (1.55);
...
%     % selected z --- now just a green grid point
%     \coordinate (z) at (0.6,0.6);
%     \node[circle, fill=green!60!black, draw=green!50!black, inner sep=2.2pt] at (z) {};
%     \node[lab, below left=1pt, text=green!50!black] at (z) {$z$};
...
%         (ann.west) to[out=200,in=25] ($(z)+(0.08,0.03)$);
% \end{tikzpicture}

% \end{frame}


% \begin{frame}{The Algorithm: 3. Local Minimization}

% \begin{itemize}
%     \item When $z$ is sufficiently close to the spectrum seek local minimizers of $\Phi_n(\cdot, A)$ over the set of $w \in \text{Grid}(M)$ that are at a distance of at most $d_{n,z}$ from $z.$ 
%     \vspace{0.5em}
%     \item The collection of local minimizers is the approximation of $\text{Sp}(A).$
% \end{itemize}

% % When $z$ is sufficiently close to the spectrum, seek local minimizers of $\gamma(\cdot, A)$ over the set of $w \in \text{Grid}(M)$ that are at a distance of at most $d_{n,z}$ from $z.$ 

% % \vspace{1em}

% % The collection of local minimizers is the approximation of $\text{Sp}(A).$

% \vspace{0.5em}

% \centering
% \begin{tikzpicture}[scale=1]
% \path[use as bounding box] (-5.2,-2.0) rectangle (5.2,2.2);

%     \path[use as bounding box] (-3.0,-2.1) rectangle (6.4,2.25);

%     % colors
%     \colorlet{annotblue}{blue}
%     \colorlet{orangeann}{orange!85!black}

%     % main disc
%     \draw[dashed, gray!60, thick] (0,0) circle (1.55);

%     % axes
%     \draw[thick, gray!60, ->] (-2.2,0) -- (2.35,0);
%     \draw[thick, gray!60, ->] (0,-1.9) -- (0,2.1);
...
%             \pgfmathsetmacro{\r}{\x*\x+\y*\y}
%             \ifdim \r pt < 2.41pt
%                 \node[circle, fill=gray!65, inner sep=1.4pt] at (\x,\y) {};
%             \fi
%         }
%     }

%     % true spectrum
%     \node[circle, fill=red!85!black, inner sep=2pt] at (-1.05,1.15) {};
%     \node[circle, fill=red!85!black, inner sep=2pt] at (-1.18,0.78) {};
%     \node[circle, fill=red!85!black, inner sep=2pt] at (-0.72,0.95) {};

%     \draw[red!85!black, thick]
%         (-0.35,-1.25)
%         .. controls (-0.20,-0.70) and (0.25,-0.55) ..
%         (0.82,-0.45)
%         .. controls (1.20,-0.38) and (1.22,-0.12) ..
%         (1.42,0.25);

%     % chosen z
%     \coordinate (z) at (0,0);
%     \node[circle, fill=green!60!black, draw=green!40!black, inner sep=2.5pt] at (z) {};
%     \node[text=green!50!black, font=\small] at (0.20,-0.28) {$z$};

%     % local search ball
%     \draw[annotblue, thick, dashed] (z) circle (0.72);
%     \node[text=annotblue, font=\scriptsize] at (0.04,0.88) {$B(z,d_{n,z})$};

%     % selected local minimizer
%     \coordinate (w) at (0,-0.6);
%     \node[circle, fill=orangeann, draw=orangeann, inner sep=2.5pt] at (w) {};
%     \node[text=orangeann, font=\small] at (-0.34,-0.42) {$w_*$};

%     % orange explanatory text
%     \node[
%         text=orangeann,
%         align=left,
%         anchor=west,
%         font=\small,
%         text width=4.6cm
%     ] (orangeText) at (2.25,-0.25)
%     {
%         Choose the $w_*$ that \\ minimizes $\Phi_n(\cdot, A)$  over \\
%         $B(z,d_{n,z})\cap \operatorname{Grid}(M).$
%     };

%     % orange arrow ending near the search ball
%     \coordinate (orangeend) at ($(z)+(280:0.80)$);

%     \draw[->, orangeann, thick]
%         (orangeText.west)
%         .. controls (2.00,-0.35) and (1.05,-1.05) ..
%         (orangeend);
...
\centering
\only<1>{ \Large 1. Discretize the complex plane}
\only<2>{ \Large 2. Solve auxiliary eigenproblems}
\only<3>{ \Large 3. Minimize locally near the spectrum}
\end{overlayarea}
...
% Bounding box for main diagram only
...
% overlay 1 labels
\node<1>[black!55] at (1.05,1.82) {$\operatorname{Grid}(n)$};
% \node<1>[text=red!80!black, font=\small] at (-1.15,1.55) {$\operatorname{Sp}(A)$};
\node<1>[
    text=red!80!black,
    font=\small,
    anchor=east
...
% overlay 2 selected z
\coordinate (zstep) at (0.6,0.6);

\node<2>[testpt] at (zstep) {};
\node<2>[lab, below left=1pt, text=green!50!black] at (zstep) {$z$};
...
\draw<2>[overlay, ->, green!60!black, thick]
...
\node<3>[text=green!50!black, font=\small] at (0.20,-0.28) {$z$};

\draw<3>[annotblue, thick, dashed] (z) circle (0.72);
\node<3>[text=annotblue, font=\scriptsize] at (0.04,0.88) {$B(z,d_{n,z})$};
...
\node<3>[text=orangeann, font=\small] at (-0.34,-0.42) {$w_*$};
...
\draw<3>[overlay, ->, orangeann, thick]
    (orangeText.west)
    .. controls (2.00,-0.35) and (1.05,-1.05) ..
    (orangeend);
...
\only<2>{
\begin{itemize}
    \item For each $z \in \text{Grid}(n),$ compute an approximation $\Phi_n(z, A) \approx \gamma(z, A)$ and the associated bound on the distance to the spectrum, $d_{n,z}.$
    \item This often requires approximating the smallest eigenvalues of the $n$-dimensional discretizations of $(A-zI)^*(A-zI)$ and $(A-zI)(A-zI)^*.$
\end{itemize}
}
...
\vspace{0.25em}
\begin{itemize}
\item When $z$ is sufficiently close to the spectrum, seek local minimizers of $\Phi_n(\cdot, A)$ over the set of $w \in \text{Grid}(n)$ that are at a distance of at most $d_{n,z}$ from $z.$ 
\item The collection of local minimizers is the approximation of $\text{Sp}(A).$
\end{itemize}
}
...
\begin{enumerate}
    \item To ease the computational effort of solving an eigenproblem at each $z \in \text{Grid}(n),$ we will explore and implement multilevel methods for eigenproblems.
    \vspace{0.5em}
...
\begin{enumerate}
    ...
    \vspace{1em}
    \item \textbf{Implement an adaptive multilevel method }for the eigensolve at each $z \in \text{Grid}(n).$ This solver will efficiently compute a bound on $\text{dist}(z, \text{Sp}(A))$ to within a guaranteed error tolerance.
\end{enumerate}
\end{column}
...
\begin{itemize}
    ...
    \item Learning about the discretization and computation of the spectra of operators using the finite element method. 
\end{itemize}
