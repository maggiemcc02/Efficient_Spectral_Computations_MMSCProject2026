...
h = 0.02
n = 1/h
grid = np.append(np.arange(0.01, 2.5, h),[2.5]) # Patrick makes this a list
phi_vals = []
DWR_phi_vals = []
DWR_errors = []
...
# print()
# print(GREEN % f'Step 1 - Compute Phi at Each Grid Point')
# print(GREEN % f'-'*100)
# print()
# print()
...
for curr_z in grid:
    ...
    print(GREEN % f"Primal eigvals: {sorted(primal_solver.eigenvalue(i) for i in range(primal_nconv))}")
    ...
    for k in range(primal_nconv):
        ...
        VTKFile(f"{z_dir_primal}/eig-{k}.pvd").write(Eout, Hout, time=float(curr_z))
    ...
    # Should I use interpolate or project?
    uh_lift = Function(Zp)
    if hcurl:
    ...
    # check for multiplicity
    print(GREEN % f"Enriched eigvals: {sorted(enriched_solver.eigenvalue(i) for i in range(enriched_nconv))}")
    ...
    # WANRNING - THE DWR ESTIMATE THAT FOLLOWS IS INCORRECT BECAUSE IT DOES NOT ACCOUNT FOR MULTIPLICITY!!
    # THIS MULTIPLICITY ISSUE NEEDS TO BE ADDRESSED!!
    ...
    # It kinda sucks that I have to do this in Firedrake). 
    # See ValueError comment below
    IhIh_up = Function(Zp)
    if hcurl:
    ...
    # Compute the primal residual (in the enriched space)
    Eh, Hh = split(uh_lift)
    Eph, Hph = split(wp)
