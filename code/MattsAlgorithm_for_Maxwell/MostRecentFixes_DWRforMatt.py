...
# CHANGES SINCE UMBERTO
######################################################################################

# Made sure the least squares solution is normalized

# General clean up of code (took out some useless stuff)

# Change uh_lift to uh in the residual assembly   

# Swapped coordinates in K matrix -> Kij = <basis_j, basis_i>. I think this matters for complex numbers (conjugate in inner produc)

# Added a multiplicity comparison plot 
...
# Should I be using uh or uh_lift

# eigenfunction(0)[0] does what? should I be using it?
...
primal_solver = LinearEigensolver(primal_problem, n_evals=5, solver_parameters=sp)
...
ap = (
    ...
    + inner(rot_v(Hp), rot_v(Gp))*dx
      # -2 z A terms
    - 2 * z * inner(complex_unit*rot_v(Hp), Fp)*dx
    + 2 * z * inner(complex_unit*rot_s(Ep), Gp)*dx
      # |z|^2 M terms
...
# solver
enriched_solver = LinearEigensolver(enriched_problem, n_evals=10, solver_parameters=sp)
...
h = 0.02
n = 1/h
grid = np.append(np.arange(0.8, 4.0, h),[4.0]) # Patrick makes this a list
multiplicities = []
smallest_eigvals = []
phi_vals = []
DWR_phi_vals = []
...
for curr_z in grid:
    ...
    smallest_eigvals.append(min_eigval) # save the compute lambda_h 
    ...
    print()
    print('Primal eigvals:', sorted(primal_solver.eigenvalue(i) for i in range(primal_nconv)))
    print()
    mult_tol = 1e-2
    mult_lst_primal = []
    for val in sorted(primal_solver.eigenvalue(i) for i in range(primal_nconv)):
        if abs(val - min_eigval) < mult_tol:
            mult_lst_primal.append(val)
    mult_count_primal = len(mult_lst_primal)


    # Save all the eigenfunctions

    z_dir_primal = f"recent_output/mixed/primal_problem_N-{N}/z-{float(curr_z):.6f}"
    os.makedirs(z_dir_primal, exist_ok=True)

    for k in range(primal_nconv):

        # Pull the eigenfunction
        eigfun = primal_solver.eigenfunction(k)[0]

        # Normalize
        Ek, Hk = split(eigfun)
        m_val = assemble((inner(Ek, Ek) + inner(Hk, Hk))*dx)
        eigfun.assign(eigfun / sqrt(m_val))

        # Write mixed components separately
        Eout, Hout = eigfun.subfunctions

        Eout.rename(f"E_primal_{k}")
        Hout.rename(f"H_primal_{k}")

        VTKFile(f"{z_dir_primal}/primal_eig-{k}.pvd").write(Eout, Hout, time=float(curr_z))
    ...
    # check for multiplicity
    print()
    print('Enriched eigvals:', sorted(enriched_solver.eigenvalue(i) for i in range(enriched_nconv)))
    print()
    mult_lst_enriched = []
    min_eigval_enriched = enriched_solver.eigenvalue(0)
    for val in sorted(enriched_solver.eigenvalue(i) for i in range(enriched_nconv)):
        if abs(val - min_eigval_enriched) < mult_tol:
            mult_lst_enriched.append(val)
    mult_count_enriched = len(mult_lst_enriched)



    # save all the enriched eigenfunctions
    
    z_dir_enriched = f"recent_output/mixed/enriched_problem_N-{N}/z-{float(curr_z):.6f}"
    os.makedirs(z_dir_enriched, exist_ok=True)

    for k in range(enriched_nconv):

        # Pull the eigenfunction
        eigfun = enriched_solver.eigenfunction(k)[0]
        ...
        VTKFile(f"{z_dir_enriched}/eig-{k}.pvd").write(Eout, Hout, time=float(curr_z))
    ...
    # Lets compare the multiplicities:

    print(BLUE % f"primal multiplicity: {mult_count_primal}")
    print(BLUE % f"enriched multiplicity: {mult_count_enriched}")

    if mult_count_primal != mult_count_enriched:
       print(RED % f"WARNING - the multiplicities of the primal and enriched problems are different!")

    # The enriched multiplicity should be more accurate so we will take that one for the least squares problem
    mult = mult_count_enriched
    multiplicities.append(mult)
    uh_basis_E = [primal_solver.eigenfunction(i)[0].subfunctions[0] for i in range(mult)]
    uh_basis_H = [primal_solver.eigenfunction(i)[0].subfunctions[1] for i in range(mult)]
    enriched_basis_E = [enriched_solver.eigenfunction(i)[0].subfunctions[0] for i in range(mult)]
    enriched_basis_H = [enriched_solver.eigenfunction(i)[0].subfunctions[1] for i in range(mult)]


    print(BLUE % f"We will take the enriched space multiplicity: {mult}")
    ...
    for i in range(mult):
        for j in range(mult):
            # K[i, j] = assemble(inner(enriched_basis_E[i], enriched_basis_E[j])*dx) + assemble(inner(enriched_basis_H[i], enriched_basis_H[j])*dx) 
            K[i, j] = assemble(inner(enriched_basis_E[j], enriched_basis_E[i])*dx) + assemble(inner(enriched_basis_H[j], enriched_basis_H[i])*dx) 

    # Assemble the F vector
    F = np.zeros((mult, 1), dtype=complex)
    for i in range(mult):
        F[i, 0] = assemble(inner(uh_basis_E[0], enriched_basis_E[i])*dx) + assemble(inner(uh_basis_H[0], enriched_basis_H[i])*dx)
    
    # Solve the least squares numpy problem
    X = np.linalg.solve(K, F)

    # Reassemble the needed enriched basis function
    chosen_enriched = Function(Zp)
    for i in range(mult):
        basis_coeff = complex(X[i,0])
        #import pdb; pdb.set_trace()
        chosen_enriched.subfunctions[0].interpolate(chosen_enriched.subfunctions[0] + basis_coeff * enriched_basis_E[i])
        chosen_enriched.subfunctions[1].interpolate(chosen_enriched.subfunctions[1] + basis_coeff * enriched_basis_H[i])

    # Save this 
    z_dir_ls = f"recent_output/mixed/least_squares_soln_N-{N}/z-{float(curr_z):.6f}"
    os.makedirs(z_dir_ls, exist_ok=True)

    # for k in range(enriched_nconv): # take this loop out later
    #     # Normalize
    #     Ek, Hk = split(chosen_enriched)
    #     m_val = assemble((inner(Ek, Ek) + inner(Hk, Hk))*dx)
    #     eigfun.assign(chosen_enriched / sqrt(m_val))
    #     # Write mixed components separately
    #     Eout, Hout = chosen_enriched.subfunctions
    #     Eout.rename(f"E_primal_{k}")
    #     Hout.rename(f"H_primal_{k}")
    #     VTKFile(f"{z_dir_ls}/least_squares_eig-{k}.pvd").write(Eout, Hout, time=float(curr_z))

   # normalize the chosen enriched function
    E_chosen_up, H_chosen_up = split(chosen_enriched)
    m_val_chosen_up = assemble((inner(E_chosen_up, E_chosen_up) + inner(H_chosen_up, H_chosen_up))*dx)
    if abs(m_val_chosen_up) < 1e-12:
      print(RED % f"WARNING - m(u, u) is really small!!")
    chosen_enriched.assign(chosen_enriched / sqrt(m_val_chosen_up))
    # Write mixed components separately
    Eout, Hout = chosen_enriched.subfunctions
    Eout.rename(f"E_primal_{k}")
    Hout.rename(f"H_primal_{k}")
    VTKFile(f"{z_dir_ls}/least_squares_result.pvd").write(Eout, Hout, time=float(curr_z))
    ...
    Ih_enriched = Function(Z)
    if hcurl:
      Ih_enriched.project(chosen_enriched, bcs=bc)
    else:
      Ih_enriched.interpolate(chosen_enriched, Z)

    # Form wp
    #wp = Function(Zp).assign(uph -  IhIh_up)
    #wp = Function(Zp).assign(uph -  Ih_up) # wont work - ValueError: All functions in the expression must have the same element as the assignee
    wp = Function(Zp).interpolate(chosen_enriched - Ih_enriched)

    # Compute the primal residual
    # Eh, Hh = split(uh_lift) #uh_lift is uh lifted to the enriched space - do I need to?
    Eh, Hh = split(uh) 
    Eph, Hph = split(wp) #pull out real part
    ...
    a_primal = (
        ...
        + inner(rot_v(Hh), rot_v(Hph))*dx
          # -2 z A terms
        - 2 * z * inner(complex_unit*rot_v(Hh), Eph)*dx
        + 2 * z * inner(complex_unit*rot_s(Eh), Hph)*dx
          # |z|^2 M terms
...
os.makedirs("recent_output/mixed/", exist_ok=True)
plt.xlabel(r"$z$")
plt.title(rf"Approximations of $\Phi_n(z, A)$ and $\omega$ ($N = {N}$)")
plt.legend()
plt.savefig(f"recent_output/mixed/most_recent_phi_dwr_plot_{N=}.pdf")
plt.close()

# plot the multiplicities

# plot the computed multiplicities
plt.plot(grid, multiplicities, 'o', label = r"computed multiplicities")
# plot the midpoints to indicate where the multiplicities should change
plt.axvline(x=midpoints[0], color='r', linestyle='--', label = r"midpoints of exact $\omega$")
for midpoint in midpoints[1:]:
    plt.axvline(x=midpoint, color='r', linestyle='--')
plt.xlabel(r"$z$")
plt.title(rf"Computed multiplicities of the folded eigenvalues ($N = {N}$)")
plt.legend()
plt.savefig(f"recent_output/mixed/most_recent_multiplicities_plot_{N=}.pdf")
plt.close()
