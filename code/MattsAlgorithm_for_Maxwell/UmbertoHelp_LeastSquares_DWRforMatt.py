...
for curr_z in grid:
    ...
    z_dir_primal = f"output/mixed/primal_problem/z-{float(curr_z):.6f}"
    ...
    z_dir_enriched = f"output/mixed/enriched_problem/z-{float(curr_z):.6f}"
    ...
    if mult_count_primal != mult_count_enriched:
       ...
       # take the smaller multiplciity 
    #    mult = max(mult_count_primal, mult_count_enriched)
    mult = mult_count_enriched # take the enriched multiplicity which should be more accurate 
    ...
    print(BLUE % f"We will take the smaller multiplicity: {mult}")
    ...
    # Save this 
    z_dir_ls = f"output/mixed/least_squares_soln/z-{float(curr_z):.6f}"
...
plt.legend()
plt.savefig(f"output/mixed/phi_dwr_plot_{N=}.pdf")
plt.close()